...
def prepareSpectralTemplateLibrary(config, lmin, lmax, velscale, LSF_Data, LSF_Templates, module_used, sortInGrid):
    ...
    # SSP model library
    template_file = config[module_used].get("TEMPLATE_FILE", 'ssp_final_mistv2.5_c3kv2.3vt10allfal_highres_nGIST_geckos_260107.fits')

    sp_models = glob.glob(os.path.join(config['GENERAL']['TEMPLATE_DIR'], config[module_used]["LIBRARY"], template_file))
