...
def run_ppxf(
    ...
    """
    Per-bin fit:
      Steps 0-3  -- pPXF over the FULL (nAges x nMetal x nAlpha) template grid
                    for age, metallicity, and first [alpha/Fe] estimate.
      Step 4     -- identify surviving (age, met) pairs by marginalising over alpha;
                    compute pPXF mean alpha as Gaussian prior centre.
      Step 5     -- rebuild multi-alpha templates at surviving (age, met) points.
      Steps 6-7  -- convolve to sigma_min; pseudo-continuum normalise Mgb region.
      Step 8     -- 1-D EMCEE FIF over alpha with Gaussian prior.
    """
    ...
    try:
        if len(optimal_template_in) > 1:
            ...
            if config["SFH"]["OPT_TEMP"] == "default":
                reshaped_templates = templates_full.reshape((templates_full.shape[0], ncomb))
            ...
            if config["SFH"]["DUST_CORR"] == True:
                ...
                dust_step3  = [{"start": [Av], "bounds": [[0, 8]],
                                 "component": component_true_step3, "fixed": [True]}]
            else:
                dust_step12 = None
                dust_step3  = None
            ...
            # Step 3: science fit over the full (nAges x nMetal x nAlpha) template grid
            pp = ppxf(templates_full.reshape(templates_full.shape[0], ncomb),
                       log_bin_data, noise_new, velscale, start,
                       ...
                       component=component_step3, dust=dust_step3)

        # Step 4: identify surviving (age, metallicity) grid points by
        # marginalising the 3-D weight cube over the alpha dimension
        weights_3d = pp.weights.reshape(nAges, nMetal, nAlpha) / pp.weights.sum()
        weights_age_met = weights_3d.sum(axis=2)  # (nAges, nMetal)
        survive_age_idx, survive_met_idx = np.where(weights_age_met > 0)
        n_survive = len(survive_age_idx)

        # pPXF luminosity-weighted mean [alpha/Fe] -- Gaussian prior centre for FIF
        alpha_ppxf = float(np.sum(weights_3d * alpha_grid) / np.sum(weights_3d))
        ...
        alpha_fif, alpha_fif_lo, alpha_fif_hi = run_fif_emcee_1d(
            ...
            alpha_ppxf, prior_sigma, nwalkers_fif, nchain_fif)
        ...
        for k in range(n_survive):
            w_full[survive_age_idx[k], survive_met_idx[k], alpha_idx_fif] = \
                weights_age_met[survive_age_idx[k], survive_met_idx[k]]
        ...
        if doplot:
            ...
            # weighted mean age and metallicity from step-3 weights (marginalised over alpha)
            mean_age_step3 = np.sum(weights_3d * 10**logAge_grid) / np.sum(weights_3d)
            mean_met_step3 = np.sum(weights_3d * metal_grid) / np.sum(weights_3d)
