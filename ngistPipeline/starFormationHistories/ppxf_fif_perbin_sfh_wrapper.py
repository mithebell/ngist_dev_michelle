...
def run_ppxf(
    ...
    try:
        ...
        # Step 6: galaxy is at (LSF_Data + sigma_kin) resolution -- no convolution
        # needed since the per-bin model convolution below matches the model to it.
        wave_gal_full = np.exp(logLam)
        galaxy_conv = log_bin_data
        noise_conv  = noise_new
        mgb_res_flag = 0
...
def save_sfh(
    ...
    # MGB_RES_FLAG unused in per-bin mode (always 0)
...
def extractStarFormationHistories(config):
    ...
    if config["SFH"]["FIXED"] == True:
        ...
        for i in range(nbins):
            ...
            # per-bin convolution: no common sigma target needed
        ...
        pass  # per-bin convolution; no sigma_max needed
    ...
    # Pre-convolve templates from LSF_Templates to LSF_Data only.
    # Per-bin kinematic broadening is then applied inside run_ppxf.
    native_fwhm_temp = LSF_Templates(wave_temp_full)
    target_fwhm_temp = lsf_data_at_temp
    ...
    printStatus.running(f"Convolving {ncomb} templates to match data LSF...")
    ...
    printStatus.updateDone(f"Convolving {ncomb} templates to match data LSF",
                            progressbar=False)
    ...
    def _call(ii):
        return run_ppxf(
            ...
            mgb_bands, nwalkers_fif, nchain_fif, alpha_values)
